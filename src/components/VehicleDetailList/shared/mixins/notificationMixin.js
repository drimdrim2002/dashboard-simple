/**
 * 알림 시스템 관련 Mixin
 * VehicleDetailList에서 사용하는 다양한 알림 기능을 제공합니다.
 */
...
export const notificationMixin = {
  methods: {
     ...
     */
    showMessage(message, type = NOTIFICATION_TYPES.INFO) {
      console.log(`${this.getLogIcon(type)} ${message}`);

      // alert 대신 비침투적인 알림 방식 사용
      if (
        type === NOTIFICATION_TYPES.ERROR ||
        type === NOTIFICATION_TYPES.WARNING
      ) {
        // DOM 이벤트 루프를 차단하지 않도록 setTimeout으로 지연 실행
        setTimeout(() => {
          // 간단한 토스트 스타일 알림 (alert 대신)
          this.showToastMessage(message, type);
        }, DEFAULT_CONFIG.NOTIFICATION_STYLE_DELAY);
      }
    },
    ...
    showToastMessage(message, type) {
      // 알림 방식 선택 (옵션: 'modal', 'banner', 'snackbar')
      const notificationType = NOTIFICATION_DISPLAY_TYPES.SNACKBAR; // 기본값: snackbar (변경 가능)

      switch (notificationType) {
        case NOTIFICATION_DISPLAY_TYPES.MODAL:
          this.showModalMessage(message, type);
          break;
        case NOTIFICATION_DISPLAY_TYPES.BANNER:
          this.showBannerMessage(message, type);
          break;
        case NOTIFICATION_DISPLAY_TYPES.SNACKBAR:
        default:
    ...
    showModalMessage(message, type) {
      ...
      modal.innerHTML = `
        <div class="notification-modal-content">
          <div class="notification-modal-header">
            ...
            <h6 class="notification-title">${this.getTypeTitle(type)}</h6>
      ...
      }, DEFAULT_CONFIG.NOTIFICATION_AUTO_HIDE_DELAY);
    },
     ...
     * 알림 타입별 제목 반환
     * @param {string} type - 알림 타입
     * @returns {string} 한국어 제목
     */
    getTypeTitle(type) {
      return NOTIFICATION_TITLES[type] || "알림";
    },

    /**
     * 알림 타입별 색상 테마 반환
     * @param {string} type - 알림 타입
     * @returns {Object} 색상 객체 { bg, text, border }
     */
    getNotificationColors(type) {
      return (
        NOTIFICATION_COLORS[type] ||
        NOTIFICATION_COLORS[NOTIFICATION_TYPES.INFO]
      );
    },
    ...
    getLogIcon(type) {
      return (
        NOTIFICATION_ICONS[type] || NOTIFICATION_ICONS[NOTIFICATION_TYPES.INFO]
      );
    },
